...
| :----------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `column`                 | 指定数据库表对应的列名                                                                                                                                                                                                                                                                                                                 |
| `type`                   | 列数据类型，推荐使用兼容性好的通用类型（所有数据库均支持：bool、int、uint、float、string、time、bytes），可搭配 `not null`、`size`、`autoIncrement` 等标签使用。也支持指定数据库专属完整数据类型，例如：`MEDIUMINT UNSIGNED not NULL AUTO_INCREMENT`、`varbinary(8)`                                                                 |
| `serializer`             | 指定数据序列化/反序列化方式，用于数据与数据库存储格式的转换，例如：`serializer:json`、`serializer:gob`、`serializer:unixtime`                                                                                                                                                                                                         |
| `size`                   | 定义列数据类型的大小或长度，例如：`size:256`（无需多余空格）                                                                                                                                                                                                                                                                           |
| `primaryKey`             | 将当前字段对应的列定义为数据库表的主键                                                                                                                                                                                                                                                                                                 |
| `unique`                 | 将当前字段对应的列定义为唯一键，确保该列无重复值                                                                                                                                                                                                                                                                                       |
| `default`                | 定义列的默认值，迁移时会同步到数据库表结构中                                                                                                                                                                                                                                                                                           |
| `precision`              | 指定列的精度（通常用于小数类型字段，搭配 `scale` 使用）                                                                                                                                                                                                                                                                                 |
| `scale`                  | 指定列的小数位数（刻度，通常用于小数类型字段，搭配 `precision` 使用）                                                                                                                                                                                                                                                                   |
| `not null`               | 指定列的值不可为 NULL，迁移时会添加 `NOT NULL` 约束                                                                                                                                                                                                                                                                                     |
| `autoIncrement`          | 指定列为自动增长列，通常与主键搭配使用（仅支持支持自增的字段类型）                                                                                                                                                                                                                                                                     |
| `autoIncrementIncrement` | 控制自动增长列的步长，即连续两条记录的自增值之间的间隔                                                                                                                                                                                                                                                                                 |
| `embedded`               | 嵌套字段，将另一个结构体的字段嵌入到当前模型中，作为当前表的列                                                                                                                                                                                                                                                                         |
| `embeddedPrefix`         | 为嵌入字段（`embedded`）的列名添加统一前缀，避免列名冲突                                                                                                                                                                                                                                                                               |
| `autoCreateTime`         | 追踪记录创建时间，对于 `int` 类型字段，默认记录秒级时间戳；可指定 `nano`/`milli` 记录纳秒/毫秒级时间戳，例如：`autoCreateTime:nano`                                                                                                                                                                                                   |
| `autoUpdateTime`         | 追踪记录创建和更新时间，对于 `int` 类型字段，默认记录秒级时间戳；可指定 `nano`/`milli` 记录纳秒/毫秒级时间戳，例如：`autoUpdateTime:milli`                                                                                                                                                                                                 |
| `index`                  | 根据参数创建索引，多个字段使用相同索引名称则创建复合索引，详情参考 [GORM 索引文档](https://gorm.io/zh_CN/docs/indexes.html)                                                                                                                                                                                                                                                                 |
| `uniqueIndex`            | 功能与 `index` 一致，区别是创建的是唯一索引，确保索引列无重复值                                                                                                                                                                                                                                                                         |
| `check`                  | 创建字段检查约束，限制列的取值范围，例如：`check:age > 13`，详情参考 [GORM 约束文档](https://gorm.io/zh_CN/docs/constraints.html)                                                                                                                                                                                                                                                                 |
| `<-`                     | 设置字段写入权限：`<-:create`（仅创建时可写入）、`<-:update`（仅更新时可写入）、`<-:false`（无写入权限）、`<-`（创建和更新均允许写入，默认）                                                                                                                                                                                          |
| `->`                     | 设置字段读取权限：`->:false`（无读取权限，查询结果不返回该字段）、`->`（正常读取权限，默认）                                                                                                                                                                                                                                          |
| `-`                      | 忽略字段相关权限/操作：`-`（无读写权限）、`-:migration`（正常读写，仅迁移时忽略该字段，不创建对应列）、`-:all`（无读写权限，且迁移时忽略）                                                                                                                                                                                            |
| `comment`                | 迁移时为字段添加注释，同步到数据库表结构的字段注释中                                                                                                                                                                                                                                                                                   |
| `foreignKey`             | 指定当前模型的字段作为连接表的外键，用于建立模型间的关联关系                                                                                                                                                                                                                                                                           |
| `references`             | 指定引用表的列名，当前模型的外键将映射到该引用列，建立关联关系的对应绑定                                                                                                                                                                                                                                                                 |
| `polymorphic`            | 指定多态类型，通常为模型名，用于建立多态关联关系                                                                                                                                                                                                                                                                                       |
| `polymorphicValue`       | 指定多态关联的对应值，默认使用表名，可手动指定自定义值                                                                                                                                                                                                                                                                                 |
| `many2many`              | 用于多对多关联，指定关联关系的连接表表名                                                                                                                                                                                                                                                                                               |
| `joinForeignKey`         | 指定连接表中的外键列名，该列将映射到当前模型的主键（或指定字段）                                                                                                                                                                                                                                                                       |
| `joinReferences`         | 指定连接表中的外键列名，该列将映射到引用模型的主键（或指定字段）                                                                                                                                                                                                                                                                       |
| `constraint`             | 配置关联关系的约束规则，例如：`OnUpdate:CASCADE`（更新时级联操作）、`OnDelete:SET NULL`（删除时设为 NULL），用于控制关联数据的联动行为                                                                                                                                                                                                  |
